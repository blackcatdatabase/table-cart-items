 ...
 */
final class CartItemsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CartItemRepository $cartItemRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'cart_items' => $this->cartItemRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\CartItems\ModuleException("Record {$idStr} not found for locking.");
            }
