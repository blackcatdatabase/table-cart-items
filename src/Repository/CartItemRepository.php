...
final class CartItemRepository implements RepoContract, KeysetRepoContract
{
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\CartItems\Definitions::table());
    ...
    */
    ...
    public function getByTenantIdAndCartIdAndBookIdAndSku(int $tenantId, string $cartId, int $bookId, string $sku, bool $asDto = false): array|\BlackCat\Database\Packages\CartItems\Dto\CartItemDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'cart_id' => $cartId, 'book_id' => $bookId, 'sku' => $sku ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CartItems\Mapper\CartItemDtoMapper::fromRow($row);
    }
    ...
    public function getIdByTenantIdAndCartIdAndBookIdAndSku(int $tenantId, string $cartId, int $bookId, string $sku) {
        $row = $this->getByTenantIdAndCartIdAndBookIdAndSku($tenantId, $cartId, $bookId, $sku);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CartItems\Dto\CartItemDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\CartItems\Dto\CartItemDto|null {
