...
namespace BlackCat\Database\Packages\CartItems\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\CartItems\Definitions;
use BlackCat\Database\Packages\CartItems\Criteria;
use BlackCat\Database\Packages\CartItems\Dto\CartItemDto as Dto;
use BlackCat\Database\Packages\CartItems\Mapper\CartItemDtoMapper as RowMapper;
...
use BlackCat\Database\Packages\CartItems\Repository\CartItemRepositoryInterface;
...
class CartItemRepository implements CartItemRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\CartItems\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\CartItems\Definitions::class);
    ...
    public function existsByTenantIdAndCartIdAndBookIdAndSkuNorm(int $tenantId, string $cartId, int $bookId, string $skuNorm): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'cart_id') . ' = :uniq_cart_id' . ' AND ' . 't.' . Ident::q($this->db, 'book_id') . ' = :uniq_book_id' . ' AND ' . 't.' . Ident::q($this->db, 'sku_norm') . ' = :uniq_sku_norm';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_cart_id' => $cartId, 'uniq_book_id' => $bookId, 'uniq_sku_norm' => $skuNorm ]);
    }
    /** @return int|string|null */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CartItems\Dto\CartItemDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\CartItems\Dto\CartItemDto|null {
